...
# 1. 시스템 설정 및 디자인 스타일 (타이틀 변경 및 겹침 방지)
...
def inject_style():
    ...
    * {{ font-family: 'NeoHyundaiReg', sans-serif !important; }}
    ...
    /* [핵심] arrow_down 겹침 해결: 모든 아이콘 텍스트 강제 제거 */
    ...
    /* UI 디자인 요소 */
    .user-info {{ color: #666; font-size: 0.85rem; margin-bottom: 2px; }}
    .system-title {{ 
        color: #002c5f; font-size: 1.1rem; font-weight: bold; 
        margin-bottom: 20px; border-left: 5px solid #002c5f; padding-left: 15px; 
    }}
    
    /* 뉴스 카드 디자인 */
    .news-card {{ padding: 10px 5px; border-bottom: 1px solid #f2f2f2; display: flex; align-items: center; gap: 12px; }}
    .m-tag {{ 
        min-width: 90px; text-align: center; font-size: 0.72rem; 
        background: #f1f6fa; color: #002c5f; padding: 3px 8px; 
        border-radius: 2px; border: 1px solid #dce6f0;
    }}
    .n-link {{ font-size: 1rem; color: #333; text-decoration: none; word-break: keep-all; }}
