...
for kw in keywords:
    ...
    with col1:
        ...
        for item in get_naver_news(kw):
            # ✅ 매체명 출력 함수(get_kor_media_name) 적용 확인
            media_name = get_kor_media_name(item.get('originallink', item.get('link', '')))
            st.markdown(f'''<div class="news-item"><span class="media-tag">{media_name}</span><a href="{item["link"]}" target="_blank" class="title-link">{clean_text(item['title'])}</a></div>''', unsafe_allow_html=True)
    with col2:
        st.caption("🔹 구글 뉴스")
        for entry in get_google_news(kw):
            raw_t = clean_text(entry.title)
            t_part = raw_t.rsplit(" - ", 1)[0] if " - " in raw_t else raw_t
            # ✅ 구글 뉴스는 원문 링크에서 매체명 추출
            media_name = get_kor_media_name(entry.link)
            st.markdown(f'''<div class="news-item"><span class="media-tag">{media_name}</span><a href="{entry.link}" target="_blank" class="title-link">{t_part}</a></div>''', unsafe_allow_html=True)
